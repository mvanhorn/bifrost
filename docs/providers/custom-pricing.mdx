...
- **Request type filtering** — restrict an override to specific operations (chat, embeddings, image generation, etc.) or leave it empty to match all
...
| `output_cost_per_image_premium_image` | Generated image — premium |
...
| `output_cost_per_audio_per_second` | Cost per second of audio output (via `output_cost_per_second`) |
